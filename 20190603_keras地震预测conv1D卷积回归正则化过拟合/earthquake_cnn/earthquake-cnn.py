...
def build_model():
    ...
    # Conv 1
    model.add(Conv1D(filters=32, kernel_size=100, strides=10, activation='relu', input_shape=(150000, 1),kernel_regularizer=regularizers.l1(0.01),)) # (rows, 1) 150000
    # Max Pooling
    model.add(MaxPooling1D(100))
    # Conv 3
    model.add(Conv1D(64, 10, activation='relu',kernel_regularizer=regularizers.l1(0.01),))
    # Average Pooling
    model.add(GlobalAveragePooling1D())
    model.add(Dense(16, kernel_initializer='normal', activation='relu',kernel_regularizer=regularizers.l1(0.01),))
    ...
    model.compile(loss='mae', optimizer=optimizer, metrics=['mean_absolute_error'])#optimizer=adam(lr=lr)
