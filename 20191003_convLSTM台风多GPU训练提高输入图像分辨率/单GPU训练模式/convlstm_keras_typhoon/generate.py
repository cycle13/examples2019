...
# 根据训练数据集生成网络所需要的序列
def generate_seq(train_data_dir,row = 200, col = 200, seq_len=12, input_len=6, intervel=6, valid_rate=0.1):
    print('**************generate_seq**************')
    ...
    for year in years :
        ...
        print('num of temp_nei_list',len(temp_nei_list),'for year',year)#year序列内部有多个图片
        temp_wai_list.append(temp_nei_list)
    #print('temp_wai_list is ', temp_wai_list)
    print('num of temp_wai_list =', len(temp_wai_list)) #year序列个数
    typhoon_seq = []
    typhoon_seq_names = []
    temp_nei_list = []
    for i_odd in range(intervel):  # intervel  0 1 2 3 4 5
        ...
        for temp_nei_list in temp_wai_list:
            ...
            frames_np = []
            frames_names = []
            for index_filename, png_filename in enumerate(temp_nei_list):#遍历一遍year内部所有图片（已按hour从小到大排列）
                ...
                if hour_num % intervel == i_odd: # 图片名称hour对intervel的余数一致才排列进入队列
                    ...
                    #     del (frames_np[indices[-1] + seq_len:-1])
                    #     del (frames_names[indices[-1] + seq_len:-1])
                    #     index_seq = 0
                    #     index_seq = index_seq + 1
