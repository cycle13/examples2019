...
def gen(batch_size=32):
    while True:
        ...
        temp = map(lambda x: [a for a in list(x)], l_plateStr)
        #print("temp\n",temp)
        ...
        ytmp = np.array(list(map(lambda x: [M_strIdx[a] for a in x], temp2)), dtype=np.uint8)
        #print('ytmp\n', ytmp)
...
def model_build_train(lr=0.001, epochs=25, batch_size=32, model_name="model_best.h5"):
    ...
    model.fit_generator(gen(batch_size), steps_per_epoch=2000, epochs=epochs,
                  validation_data=gen(batch_size), validation_steps=1280,
...
if __name__ == "__main__":
    ...
    model_build_train(lr=0.001, epochs=1, batch_size=32, model_name="model_best.h5")
